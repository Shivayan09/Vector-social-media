...
export const addComment = async (req, res) => {
    try {
        ...
        // ── Resolve @mentions ──────────────────────────────────────────────
        // Extract unique lowercase usernames from @username tokens in content
        ...
        // Fire a "mention" notification for each mentioned user, skipping:
        //   - the commenter themselves
        //   - the post author (already received a "comment" notification above)
        for (const mentionedId of mentionedUserIds) {
            const mentionedIdStr = mentionedId.toString();

            if (
                mentionedIdStr === req.user.id ||
                mentionedIdStr === post.author.toString()
            ) {
                continue;
            }
            ...
            try {
                ...
                io.to(mentionedIdStr).emit("notification:new", {
                    notificationId: mentionNotification._id,
                    type: mentionNotification.type,
                });
            } catch (err) {
                // Swallow duplicate-key errors so one bad mention doesn't abort the request
...
export const deleteComment = async (req, res) => {
    try {
        ...
        // Also clean up any mention notifications tied to this comment
        await Notification.deleteMany({ comment: comment._id, type: "mention" });
